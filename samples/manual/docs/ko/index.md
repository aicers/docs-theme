...
[설치 절차](#설치)로 이동하면 같은 페이지 앵커가 한국어 제목으로
연결되는지 확인할 수 있습니다.

1. 테마를 `docs/theme/`에 설치합니다.
2. `mkdocs.yml`이 `INHERIT`로 베이스를 가리키게 합니다.
3. 사이트를 빌드합니다.
...
`mermaid`로 표시한 펜스 블록은 코드 블록이 아니라 가운데 정렬된
다이어그램으로 렌더링됩니다.
...
컨테이너보다 넓은 테이블은 페이지를 넘치지 않고 가로로
스크롤됩니다.

<!-- markdownlint-disable MD013 -->
| 서비스 | 리전 | 버전 | 복제본 | CPU | 메모리 | 상태 | 가동률 | 담당 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 게이트웨이 | us-east-1 | 2.4.0 | 6 | 2000m | 4Gi | 정상 | 99.98% | 플랫폼 |
| 레지스트리 | us-west-2 | 1.8.3 | 3 | 1000m | 2Gi | 정상 | 99.95% | 플랫폼 |
| 스케줄러 | eu-central-1 | 0.9.1 | 2 | 500m | 1Gi | 저하 | 99.20% | 런타임 |
