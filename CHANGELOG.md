...
- Added `brand-print.svg`, the black-lettering wordmark the PDF cover
  uses, and `brand-symbol.svg`, the cube alone. A wordmark scaled to a
  16-pixel tab icon is unreadable; the cube is not.
- Added `extra.pdf.cover_logo`, a path relative to `docs_dir`, so a
  project documenting a product with its own mark can put it on the PDF
  cover. A path that does not resolve is an error rather than a silent
  fall back to the company mark.
- Added a provenance line to the PDF cover naming the theme that rendered
  it. A PDF leaves the repository that produced it, so without it the
  artifact says nothing about what to reproduce a rendering complaint
  against. The value comes from `docs/theme/.meta`, so a drifted tree
  cannot misreport itself as a release; a `--source` install renders
  `docs-theme (local build)` rather than the version it was asked for,
  which never resolved a release. A missing `.meta` is an error.

### Changed

- The brand assets are the ClumL company mark, replacing a `brand.svg`
  that was the Clumit Security product logo — one consumer's product,
  shipped to every consumer. A project documenting a different product
  would have published another product's branding.
...
- Added `styles/base.css` content: `@font-face` for the six shipped woff2
  files and `--md-text-font: "Pretendard"`, giving Korean body typography
...
- `extra_css` now points at `theme/`, and at `theme/styles/api.css` for
...
- A single-column PDF table keeps its content width instead of being
  stretched across the page.

### Fixed

- `fetch-theme.sh` installs into `docs/theme/` instead of `docs/.theme/`.
  MkDocs excludes every dot-prefixed path from the build, so assets
  installed under the old path never reached the published site.
...
- A PDF table now carries its own grid, header shading, and cell padding.
  Material draws the grid with a custom property that only a browser's
  colour scheme defines, so in the PDF it resolved to nothing and a table
  arrived as unruled columns of text with no way to tell which cell
...
[0.3.0]: https://github.com/aicers/docs-theme/compare/0.2.0...0.3.0
[0.2.0]: https://github.com/aicers/docs-theme/compare/0.1.0...0.2.0
